...
def train_loop(env, agent, max_total_steps, max_steps, batch_size, intermediate_policies=False, path="./", verbose=False, update_all=True):
    ...
    while steps < max_total_steps:
        ...
        try:
            ...
            pass
        for step in range(max_steps):
            if verbose and steps % (max_total_steps // 100) == 0:
                print(f"Steps: {steps}")
            if steps > 2 * batch_size:
                action = agent.policy.get_action(state)
                next_state, reward, done, _ = env.step(action)
            ...
            if update_all:
                ...
                if len(agent.replay_buffer) > batch_size and steps % 10 == 0:
                    agent.update(batch_size)
            if intermediate_policies and steps % (max_total_steps // 5) == 0:
                agent.save_policy(path + f"policy{steps}.pth")
            if done:
                break
        rewards.append(ep_reward)
        if verbose:
